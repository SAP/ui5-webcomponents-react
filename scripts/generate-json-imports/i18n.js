...
const files = fs.readdirSync('assets/i18n/');
...
if (languages.length === 0) {
  ...
  const assetsImportsString = languages
    .map((key) => `import ${key} from "../assets/i18n/messagebundle_${key}.json";`)
    .join('\n');
...
fs.writeFileSync('json-imports/i18n.js', content);
