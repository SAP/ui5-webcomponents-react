...
function saveResults() {
  fs.writeFileSync(join('scripts', 'rollup', 'results.json'), JSON.stringify(currentBuildResults, null, 2));
}
  ...
  return current.bundleSizes
    .map((result) => {
      const prev = prevResults.bundleSizes.filter(
        (res) => res.filename === result.filename && res.bundleType === result.bundleType
      )[0];
      if (result === prev) {
