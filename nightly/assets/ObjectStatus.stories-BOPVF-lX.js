  ...
  render() {
    const renderStatuses = (interactive = false, inverted = false) => {
      return Object.values(States).map((state: ObjectStatusPropTypes['state']) => {
        ...
        return <ObjectStatus key={\`\${state}-\${interactive ? 'interactive' : ''}-\${inverted ? 'inverted' : ''}\`} interactive={interactive} inverted={inverted} state={state}>
            {state}
          </ObjectStatus>;
      });
    };
