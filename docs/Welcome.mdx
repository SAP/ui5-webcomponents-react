...
Popover with custom data-attribute opened by clicking a button:
...
export default function App() {
  ...
  // if you only want to type the event parameter, you can use the `Parameters` utility type.
  const handleAfterClose = (e: Parameters<PopoverPropTypes['onAfterClose']>[0]) => {
    setOpen(false);
  };
