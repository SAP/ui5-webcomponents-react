...
[//]: # 'TODO: check events details etc and note the differences'
...
[//]: # 'TODO: explanation why preserveHeaderStateOnScroll has been removed'

- `preserveHeaderStateOnScroll`: TODO
...
- `header` has been renamed to `heading` and is now a `slot`. The `font-size` isn't automatically adjusted anymore, so to keep the intended design you can leverage the new `snappedHeading` prop and apply the corresponding CSS Variables yourself.
  ...
  <DynamicPageTitle
    ...
    snappedHeading={
      <Title style={{ fontSize: ThemingParameters.sapObjectHeader_Title_SnappedFontSize }}>Snapped Header Title</Title>
    }
  />
  ```
