...
- `onClear`: The event is now a standard `Button` `onClick` event. The `detail` properties `filters` and `search` have been removed.
...
- `onGo`: The event is now a standard `Button` `onClick` event. The `detail` properties `elements`, `filters`, `search`, `nativeDetail` have been removed.
