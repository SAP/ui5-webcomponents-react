...
- `filterBarExpanded` --> `filterBarCollapsed` <br />
  Previously, `filterBarExpanded` was defaulted to `true`. This prop has been replaced by `filterBarCollapsed` with the default value of `false`.
- `showFilterConfiguration` --> `hideFilterConfiguration` <br />
  Previously, `showFilterConfiguration` was defaulted to `true`. This prop has been replaced by `hideFilterConfiguration` with the default value of `false`.
...
**Full Migration Example (removed props are not listed for brevity):**
