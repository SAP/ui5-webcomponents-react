...
<MessageStrip hideCloseButton>
  All UI5 Web Components fully support the Horizon theme family, but for custom UI5 Web Components for React components
  (e.g. ObjectPage, AnalyticalTable, etc.) it's still in development, so it could be possible to encounter discrepancies
  in the design. If you notice one please feel free to open an{' '}
  <Link href="https://github.com/SAP/ui5-webcomponents-react/issues/new/choose">issue</Link>.
</MessageStrip>
