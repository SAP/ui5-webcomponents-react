...
or the [changelog](https://github.com/SAP/ui5-webcomponents-react/blob/main/CHANGELOG.md)._
...
The `Avatar` component does not longer have an `img` prop but expects you to pass an `<img />` element as children.
...
You can open all Popovers (`ActionSheet`, `Popover`, `ResponsivePopover`) using the `.openBy` method:
...
function MyOldComponent() {
  ...
  const openPopover = (event) => {
    ref.current.open(event.target); // ActionSheet and ResponsivePopover, Popover was already using openBy
  };
  return <Popver ref={ref}>Content</Popver>;
}
...
function MyNewComponent() {
  const ref = useRef();
  const openPopover = (event) => {
    ref.current.openBy(event.target);
  };
  return <Popver ref={ref}>Content</Popver>;
}
```
