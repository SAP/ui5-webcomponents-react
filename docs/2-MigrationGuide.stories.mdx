...
The `ThemeProvider` was provided a nested styles object with the properties `theme`, `contentDensity`, `parameters` and `rtl`.
This object might have been used by you in case you where creating dynamic styles using `react-jss`. It has now been
...
// as now everything is static you can also omit the function completely:
...
const MyComponent = () => {
  ...
  useEffect(() => {
    getComputedStyle(ref.current).getPropertyValue('--_ui5_content_density'); // this return either cozy or compact
  }, []);
