...
import { dummyDataSet, dummyDiscreteDataSet, schedulingEDFData, tagDataset } from './examples/Dataset.ts';
import { TimingFigure, Tag } from './examples/Annotations.tsx';
...
Using annotations can be so powerful. This allows very good customization of the the timeline you want. The
chart can be made to contain only custom annotations that utilize the other features available in the
`TimelineChart`. The example below shows the timeline for the some of the most momentuous inventions in the
last 5000 years.
