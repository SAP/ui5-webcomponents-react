...
<Canvas>
  <Story name="Default">
    {(args) => {
      return (
        <>
          <Button id="openPopoverBtn">Open Popover (set the open prop to true)</Button>
          <ResponsivePopover {...args} />
        </>
      );
    }}
  </Story>
...
const MyComponentWithPopover = () => {
  ...
  return (
    <>
      ...
      <ResponsivePopover opener={'openPopoverBtn'} open={popoverIsOpen} />
    </>
  );
};
```
