...
We are offering those helpers methods as both hooks and static methods:

`Modals.useShowXZY`

Use this hook when you are in a React context where you are allowed to use hooks.
Calling this hook returns a memoized function, which you can call to show the popup by passing the props and an optional container.
