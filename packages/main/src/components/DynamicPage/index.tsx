 ...
 */
const DynamicPage: FC<DynamicPageProps> = forwardRef(
  (props: DynamicPageProps, ref: Ref<HTMLDivElement>) => {
    const {
      title,
      header,
      tooltip,
    ...
    const dynamicPageClasses = StyleClassHelper.of(
      classes.dynamicPage,
    ...
    if (
      headerState === HEADER_STATES.HIDDEN ||
      headerState === HEADER_STATES.HIDDEN_PINNED
    ) {
    ...
    const onToggleHeaderContentVisibility = useCallback(
      (e, element?: Element | HTMLElement) => {
        ...
        setHeaderStateRef((oldState) => {
          if (
            oldState === HEADER_STATES.VISIBLE_PINNED ||
            oldState === HEADER_STATES.HIDDEN_PINNED
          ) {
            return shouldHideHeader
              ? HEADER_STATES.HIDDEN_PINNED
              : HEADER_STATES.VISIBLE_PINNED;
          }
          headerStateResetOnScroll = true;
          return shouldHideHeader
            ? HEADER_STATES.HIDDEN
        ...
        dynamicPageRef.current.addEventListener(
          ...
          () => {
            if (
              headerStateRef.current !== HEADER_STATES.VISIBLE_PINNED &&
              headerStateResetOnScroll
            )
    ...
    const onHoverToggleButton = useCallback(
      (e) => {
        ...
        topHeaderRef.current.style.backgroundColor =
          e?.type === 'mouseover'
            ? ThemingParameters.sapTile_Active_Background
            : null;
      },
      [topHeaderRef]
    );
    ...
    const onToggleHeaderContent = (e) => {
      if (e.target.tagName === 'DIV') {
        onToggleHeaderContentVisibility(
          e,
          anchorBarRef.current.children.item(0).children.item(0)
        );
      }
    };
        ...
        <FlexBox
          ...
          style={{
            top:
              headerState === HEADER_STATES.VISIBLE_PINNED ||
              headerState === HEADER_STATES.VISIBLE
                ? (headerContentRef?.current?.offsetHeight ?? 0) +
                  topHeaderHeight
                : topHeaderHeight,
          }}
        >
