...
<Canvas>
  <Story name="Tree Table" args={{ data: generateData(3, true), isTreeTable: true }}>
    {(args) => {
      const [collapsed, setCollapsed] = useReducer((coll) => !coll, false);
      return (
        <Panel collapsed={collapsed} onToggle={setCollapsed} headerText="Show Tree Table">
          {collapsed ? (
            ...
            <AnalyticalTable
              ...
              columns={args.columns}
              isTreeTable
              tableHooks={[useIndeterminateRowSelection(true)]}
              selectedRowIds={{ '1.1': true, '1.2': false, '2.1.2.1.3.4': true }}
            />
          )}
        </Panel>
      );
