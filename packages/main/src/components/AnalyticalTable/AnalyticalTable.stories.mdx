...
Plugin hooks add features to the `Analytical Table` that are meant for special use-cases and are therefore not available per default.
...
```jsx
<AnalyticalTable data={data} columns={columns} tableHooks={useRowDisableSelection} />
```
