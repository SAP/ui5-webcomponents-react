...
export interface TableProps extends Omit<CommonProps, 'title'> {
   ...
   */
  onGroup?: (
    e: CustomEvent<{ column: unknown; groupedColumns: string[] }>
  ) => void;
  /**
   ...
   */
  onRowExpandChange?: (
    e?: CustomEvent<{ row: unknown; column: unknown }>
  ) => any;
  /**
   * Fired when the columns order is changed.
   */
  onColumnsReordered?: (
    e?: CustomEvent<{ columnsNewOrder: string[]; column: unknown }>
  ) => void;
  /**
 ...
 */
const AnalyticalTable: FC<TableProps> = forwardRef(
  (props: TableProps, ref: Ref<HTMLDivElement>) => {
    const {
      columns,
      className,
      style,
    ...
    const getSubRows = useCallback((row) => row[subRowsKey] || [], [
      subRowsKey,
    ]);
    ...
    const updateTableClientWidth = useCallback(() => {
      if (tableRef.current) {
        dispatch({
          type: 'TABLE_RESIZE',
          payload: { tableClientWidth: tableRef.current.clientWidth },
        });
      }
    }, [tableRef.current]);
    ...
    useEffect(() => {
      ...
      const tableWidthObserver = new ResizeObserver(
        debounce(updateTableClientWidth, 500)
      );
    ...
    const calcRowHeight = parseInt(
      getComputedStyle(tableRef.current ?? document.body).getPropertyValue(
        '--sapWcrAnalyticalTableRowHeight'
      ) || '44'
    );
    ...
    const onGroupByChanged = useCallback(
      (e) => {
        ...
        if (isGrouped) {
          ...
          groupedColumns = tableState.groupBy.filter(
            (group) => group !== column.id
          );
        }
    ...
    const verticalScrollBarRef: RefObject<DivWithCustomScrollProp> = useRef(
      null
    );
    ...
    return (
      ...
      >
        ...
        <FlexBox>
            ...
            {loading && props.data?.length === 0 && (
              <TablePlaceholder
                columns={tableInternalColumns.filter(
                  (col) =>
                    (col.isVisible ?? true) &&
                    !tableState.hiddenColumns.includes(col.accessor)
                )}
                rows={props.minRows}
                style={noDataStyles}
          ...
          {(tableState.isScrollable === undefined ||
