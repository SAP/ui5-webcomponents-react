...
const useHook = (toggle) => (hooks) => {
  hooks.useInstance.push((instance) => {
    const initial = useRef(true);
    useEffect(() => {
      if (!initial.current) {
        // console.log('toggle', toggle);
        // instance.toggleRowExpanded('11');
        instance.dispatch({ type: 'toggleRowExpanded', id: '11' });
      }
...
export const Test = {
  render(args) {
    ...
    const handleExpandChange = (e) => {
      // setExpanded((prev) => ({ ...prev, [e.detail.row.id]: !e.detail.row.isExpanded }));
    };
    ...
    return (
      <>
        <button
          onClick={() => {
            setToggle((prev) => !prev);
            // setExpanded((prev) => ({ ...prev, '11': i.current }));
            // console.log(tableInstanceRef.current.rowsById['11']);
            // tableInstanceRef.current.rowsById['11'].toggleRowExpanded();
            // i.current = !i.current;
          }}
        >
          Click
        ...
        <AnalyticalTable
          ...
          overscanCount={Infinity}
          onRowExpandChange={handleExpandChange}
          tableHooks={[useHook(toggle)]}
          reactTableOptions={{
