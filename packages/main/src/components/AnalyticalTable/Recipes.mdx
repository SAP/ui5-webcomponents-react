...
## How to render a Dialog or Popover inside a custom Cell?
...
- Events for i.a. the keyboard navigation are bubbling up to the modal, so in case of `focus`, `key`, `click`, etc. event issues, prevent the event from bubbling up to the modal by calling `event.stopPropagation()` in the respective handler.
