...
If a Dialog or Popover is rendered inside a cell, the `focus` event is bubbling up to the table which implements its own logic for focus handling to support keyboard navigation.
