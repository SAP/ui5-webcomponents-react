 ...
 */
const Form: FC<FormPropTypes> = forwardRef(
  (props: FormPropTypes, ref: Ref<HTMLDivElement>) => {
    const {
      title,
      children,
      className,
    ...
    const [currentRange, setCurrentRange] = useState(
      getCurrentRange('StdExt', window.innerWidth).name
    );
    ...
    const [formGroups, updatedTitle] = useMemo(() => {
      ...
      if (
        ...
        !title &&
        (children as ReactElement).props.title?.length > 0
      ) {
        return [
          cloneElement(children as ReactElement, { title: null }),
          (children as ReactElement).props.title,
        ];
      }
      ...
      const estimatedNumberOfGroupRows =
        childrenArray.length / currentColumnCount;
      for (let i = 0; i < estimatedNumberOfGroupRows; i++) {
        rows[i] = childrenArray.slice(
          i * currentColumnCount,
          i * currentColumnCount + currentColumnCount
        );
      }
      ...
      rows.forEach((rowGroup: ReactElement[], rowIndex) => {
        const numberOfRowsOfEachForm = rowGroup.map((row) => {
          ...
          return (
            Children.count(row.props.children) +
            (row.props?.title?.length > 0 ? 1 : 0)
          );
        });
    ...
    const formClassNames = StyleClassHelper.of(classes.form).putIfPresent(
      className
    );
