...
describe('FlexBox', () => {
  test('JustifyContent: End', () => {
    ...
    expect(
      window.getComputedStyle(screen.getByTestId('flexbox')).justifyContent
    ).toEqual('flex-end');
  });
  ...
  test('Height and Width', () => {
    render(
      <FlexBox
        style={{ height: '1337px', width: '42px' }}
        data-testid="flexbox"
      >
        <span>Test 1</span>
      </FlexBox>
    ...
    expect(
      window.getComputedStyle(screen.getByTestId('flexbox')).height
    ).toEqual('1337px');
    expect(
      window.getComputedStyle(screen.getByTestId('flexbox')).width
    ).toEqual('42px');
  });
  ...
  test('Display: Inline', () => {
    ...
    expect(
      window.getComputedStyle(screen.getByTestId('flexbox')).display
    ).toEqual('inline-flex');
  });
