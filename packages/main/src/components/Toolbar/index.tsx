...
} from '@ui5/webcomponents-react-base/lib/Utils';
 ...
 */
const Toolbar: FC<ToolbarProptypes> = forwardRef(
  (props: ToolbarProptypes, ref: Ref<HTMLDivElement>) => {
    const {
      children,
      toolbarStyle,
      design,
    ...
    const childrenWithRef = useMemo(() => {
      ...
      const refactoredChildren = React.Children.toArray(children).map(
        (child, index) => {
    ...
    const calculateVisibleItems = useCallback(() => {
      ...
      requestAnimationFrame(() => {
        ...
        const availableWidth = outerContainer.current.getBoundingClientRect()
          .width;
        let consumedWidth = 0;
        let lastIndex = null;
        let lastFitWidth = 0;
        ...
        if (availableWidth - OVERFLOW_BUTTON_WIDTH <= 0) {
          ...
          controlMetaData.current.forEach((item, index) => {
            const currentMeta = controlMetaData.current[index] as {
              ref: RefObject<HTMLElement>;
            };
            if (currentMeta && currentMeta.ref && currentMeta.ref.current) {
              let nextWidth = currentMeta.ref.current.getBoundingClientRect()
                .width;
              nextWidth +=
                index === 0 || index === controlMetaData.current.length - 1
                  ? 4
                  : 8; //first & last element = padding: 4px
