...
You can achieve that either by leveraging the `onOverflowChane` event and retrieve the current ID there, or by updating it inside the `click` handler of the respective opener component.
...
const ToolbarComponent = () => {
  ...
  const handleOverflowChange = (e) => {
    if (e.overflowElements?.length) {
      ...
      if (overflowId) {
        setOpener([...e.overflowElements].find((item) => item.dataset.opener)?.id);
      } else {
