...
Both components accept the same `props` as the standard `Button` component, with the difference that `icon` is now required and `children` are only rendered inside the popover.
