      ...
      visitor: {
        CallExpression(path, state) {
          ...
          if (
            ...
            t.isStringLiteral(path.node.arguments[0]) &&
            (
              path.node.arguments[0].value.includes('@ui5/webcomponents/dist')
              ...
              || path.node.arguments[0].value.includes('@ui5/webcomponents-icons/dist')
              || path.node.arguments[0].value.includes('@ui5/webcomponents-theming/dist')
            )
          ) {
            path.node.arguments[0].value = path.node.arguments[0].value.replace('/dist', '/cjs');
            path.node.arguments[0].value = path.node.arguments[0].value.replace(/\.js$/, '.cjs');
